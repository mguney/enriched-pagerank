// set up SVG for D3
var width = 960, height = 500, colors = d3.scale.category20();
...
function tick() {
    ...
    path
            .attr('d', function(d) {
                var deltaX = d.target.x - d.source.x, deltaY = d.target.y - d.source.y, dist = Math.sqrt(deltaX * deltaX + deltaY * deltaY), normX =
                        deltaX / dist, normY = deltaY / dist, sourcePadding = d.left ? 17 : 12, targetPadding = d.right ? 17 : 12, sourceX =
                        d.source.x + (sourcePadding * normX), sourceY = d.source.y + (sourcePadding * normY), targetX = d.target.x - (targetPadding * normX), targetY =
                        d.target.y - (targetPadding * normY);
...
function restart() {
    ...
    g.append('svg:circle').attr('class', 'node').attr('r', 12).style('fill', function(d) {
        ...
        drag_line.style('marker-end', 'url(#end-arrow)').classed('hidden', false).attr('d', 'M' + mousedown_node.x + ',' + mousedown_node.y + 'L'
                + mousedown_node.x + ',' + mousedown_node.y);
        ...
        // NB: links are strictly source < target; arrows separately specified
        // by booleans
